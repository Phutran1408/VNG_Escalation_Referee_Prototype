...
const PRESETS: { label: string; tag: string; tagColor: string; data: FormState }[] = [
  {
  ...
  return (
    <section id="leave-form" className="scroll-mt-6">
      <div className="card">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
            <div>
              <h2 className="font-display text-lg font-700 text-slate-900">
                Thẩm Định Đơn Xin Nghỉ Học Tức Thì (Lõi Agent SV2)
              </h2>
        ...
        {result && (
          <div
            className={`border-t p-6 transition-all ${
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
              <div className="space-y-3 flex-1">
                <div className="flex flex-wrap items-center gap-2.5">
                  <span className="text-xs font-bold font-mono-data text-slate-500 uppercase tracking-wider">
                    KẾT QUẢ PHÂN XỬ:
                  </span>
                  {result.decision === "AUTO_APPROVE" ? (
                ...
                {result.escalationQuestion && (
                  <div className="p-4 bg-amber-100/90 border-2 border-amber-300 rounded-xl shadow-xs space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-mono-data bg-amber-200/80 text-amber-900 px-2 py-0.5 rounded font-semibold">
                        Giải quyết trong 1 lượt
                      </span>
                    </div>
                    ...
                    <div className="text-[11px] text-amber-800 italic flex items-center gap-1">
                      <span>* Quy tắc bất biến SV2:</span>
                      <span>Không bao giờ xuất kết quả chắc chắn trên ca đã gắn cờ. Quyết định thuộc về con người.</span>
                    </div>
                  </div>
                )}
              </div>

              <div className="flex flex-col items-end gap-1 shrink-0 font-mono-data text-xs text-slate-400">
                <span>Mã đơn: {result.requestId}</span>
                <span>{result.timestamp.slice(0, 19)}Z</span>
              </div>
            </div>
          </div>
        )}
...
export default function App() {
  ...
  const handleVerifyResults = useCallback((results: VerifyResult[]) => {
    ...
    showToast(`Đã nạp ${results.length} ca kiểm thử vào Nhật ký quyết định.`);
  }, []);
  ...
  const handleFormResult = useCallback((res: EvaluateResponse, form: FormState) => {
    ...
    showToast(`Đã thẩm định đơn ${res.requestId}: ${res.decision}`);
  }, []);
  ...
  const handleOverride = useCallback(async (requestId: string) => {
    ...
    showToast(`Đã can thiệp (Override) quyết định cho đơn ${requestId}`);
  }, []);

  const handleClearAudit = useCallback(() => {
    if (window.confirm("Bạn có chắc chắn muốn xóa toàn bộ nhật ký kiểm duyệt hiện tại?")) {
      setAuditEntries([]);
      showToast("Đã xóa nhật ký quyết định.");
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800 antialiased font-sans">
      ...
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed bottom-5 right-5 bg-slate-900 text-white text-xs font-semibold px-4 py-2.5 rounded-lg shadow-lg border border-slate-700 flex items-center gap-2 animate-bounce">
      ...
      <footer className="border-t border-slate-200 bg-white py-6 text-center text-xs text-slate-400">
        AI Escalation Referee Prototype · Spec A (The Escalation Referee) · Bảng 1: OrganizationAI · SV2 Lõi Agent & Decision Log
      </footer>
    </div>
  );
}
