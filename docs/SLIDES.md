...
- **Nguy cơ 1 — Over-escalation (Đẩy việc thừa)**: Nếu AI quá e ngại rủi ro, mọi đơn đều bị đẩy lên Trưởng khoa/Giảng viên $\rightarrow$ Tự động hóa thất bại, không giảm tải được gì.
- **Nguy cơ 2 — Under-escalation (Ảo giác bỏ lọt sai phạm)**: Nếu LLM tự ý phê duyệt các ca chứng từ giả mạo, mờ ngày tháng, hoặc sinh viên đã nghỉ quá 20% số buổi $\rightarrow$ Vi phạm nghiêm trọng Quy chế đào tạo của Bộ GD&ĐT.
...
- **Vắng quá 20% học phần** (Loại 2) $\neq$ **Bảo lưu học kỳ** (Loại 3).
...
| :--- | :---: | :---: | :--- |
| **Tỷ lệ Over-escalation (Ca thường quy)** | $\le 5\%$ | **0.0%** (7/7 ca chuẩn) | Toàn bộ ca hợp lệ được phê duyệt tự động, giảm tải tối đa cho cấp trên. |
| **Độ chính xác trên tập Testbed chuẩn** | $\ge 90\%$ | **100%** (15/15 ca) | Đạt tuyệt đối trên cả 5 ca tiêu biểu và 10 ca mở rộng. |
...
| **Thời Gian Xử Lý Trung Bình (Latency)** | $< 1000\text{ ms}$ | **~6 ms (Rule) / ~45 ms (LLM)** | Xử lý real-time tức thời, phản hồi ngay lập tức cho người dùng. |
