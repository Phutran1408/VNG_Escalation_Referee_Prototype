#!/usr/bin/env python3
"""
Tự động chuyển đổi proposal_vi.md sang proposal_vi.pdf chuẩn phong cách học thuật cao cấp
(Mục lục có dotted leaders chuẩn Word/LaTeX, căn đều 2 bên, chuyển hóa 100% ký hiệu toán học sang Unicode, không màu sắc lòe loẹt, thuần text).
"""
...
MD_PATH = os.path.join(WORKSPACE_DIR, "proposal_vi.md")
HTML_PATH = os.path.join(WORKSPACE_DIR, "proposal_vi.html")
PDF_PATH = os.path.join(WORKSPACE_DIR, "proposal_vi.pdf")
...
def md_to_html(md_text):
    ...
    while i < len(lines):
        ...
        # Kiểm tra phần Mục lục
        if stripped == "# MỤC LỤC":
            if in_table:
                html_lines.append(flush_table())
            html_lines.append("<div class='toc-section'>")
            html_lines.append("<h1 class='toc-heading'>MỤC LỤC</h1>")
            i += 1
            while i < len(lines):
                toc_line = lines[i].rstrip()
                toc_stripped = toc_line.strip()
                if toc_stripped.startswith("# PHẦN I") or (toc_stripped.startswith("---") and i + 1 < len(lines) and lines[i+1].strip().startswith("# PHẦN I")):
                    break
                if not toc_stripped:
                    i += 1
                    continue
                ...
                # Tiêu đề phần lớn: **PHẦN I: ...**
                if toc_stripped.startswith("**PHẦN") or toc_stripped.startswith("**PHỤ LỤC"):
                    part_title = toc_stripped.replace("**", "").strip()
                    html_lines.append(f"<div class='toc-part'>{part_title}</div>")
                else:
                    # Parse mục kèm số trang
                    # Ví dụ: - 1. Quyết định điều hành ... Trang 1 hoặc   - 1.1. ... Trang 1
                    indent = len(toc_line) - len(toc_line.lstrip())
                    is_sub = indent >= 2 or toc_stripped.startswith("- 1.") and "." in toc_stripped[4:8]
                    
                    m = re.match(r"^[-\*]?\s*(.*?)\s*\.{3,}\s*(?:Trang\s*)?(\d+|[A-Za-z0-9]+)$", toc_stripped)
                    if not m:
                        m = re.match(r"^[-\*]?\s*(.*?)\s+(?:Trang\s*)(\d+|[A-Za-z0-9]+)$", toc_stripped)
...
<head>
<meta charset="UTF-8">
<title>ACADEMIC ESCALATION REFEREE (AER) — Technical Proposal</title>
...
<style>
  @page {{
    ...
    @top-right {{
      content: "ACADEMIC ESCALATION REFEREE (AER) · BẢNG 1 ORGANIZATIONAI";
      font-size: 8pt;
      font-family: 'Inter', sans-serif;
      color: #64748b;
      font-weight: 500;
  ...
  * {{
  ...
  /* Căn đều hai bên chuẩn mực học thuật */
  p {{
    margin-top: 0;
    margin-bottom: 11px;
    text-align: justify;
  ...
  /* Định dạng Mục Lục chuẩn chỉ như tài liệu học thuật */
  .toc-section {{
  ...
  /* Khối Manifesto / Trích dẫn chuẩn */
  .callout-quote {{
  ...
  /* Khối mã nguồn / JSON schema thanh lịch */
  pre.code-block {{
    background-color: #f8fafc;
    color: #0f172a;
...
def generate_pdf():
    print(f"1. Đang đọc nội dung từ {MD_PATH}...")
    with open(MD_PATH, "r", encoding="utf-8") as f:
        md_text = f.read()

    print("2. Đang chuyển đổi Markdown sang HTML (Mục lục có dotted leaders, căn đều hai bên)...")
    body_html = md_to_html(md_text)
    full_html = build_full_html(body_html)

    with open(HTML_PATH, "w", encoding="utf-8") as f:
        f.write(full_html)
    print(f"✓ Đã lưu HTML tại: {HTML_PATH}")

    browser = find_browser()
    if not browser:
        print("❌ Không tìm thấy Chrome hoặc Microsoft Edge trên máy.")
        return False

    print(f"3. Đang gọi trình duyệt biên dịch PDF: {browser}")
    cmd = [
        browser,
        "--headless=new",
        "--disable-gpu",
    ...
    if res.returncode == 0 and os.path.exists(PDF_PATH):
        ...
        print(f"🎉 XUẤT THÀNH CÔNG! File PDF đã được cập nhật chuẩn xác tại:")
