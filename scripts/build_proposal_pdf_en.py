#!/usr/bin/env python3
"""
Tự động chuyển đổi proposal_en.md sang proposal_en.pdf chuẩn phong cách học thuật cao cấp
...
MD_PATH = os.path.join(WORKSPACE_DIR, "proposal_en.md")
HTML_PATH = os.path.join(WORKSPACE_DIR, "proposal_en.html")
PDF_PATH = os.path.join(WORKSPACE_DIR, "proposal_en.pdf")
    ...
    while i < len(lines):
        ...
        if stripped == "# TABLE OF CONTENTS":
            ...
            while i < len(lines):
                ...
                if toc_stripped.startswith("# PART I") or (toc_stripped.startswith("---") and i + 1 < len(lines) and lines[i+1].strip().startswith("# PART I")):
                    break
                if not toc_stripped:
                    i += 1
                    continue
                ...
                if toc_stripped.startswith("**PART") or toc_stripped.startswith("**APPENDICES"):
...
<style>
  @page {{
    ...
    @top-right {{
      content: "ACADEMIC ESCALATION REFEREE (AER) · TRACK 1 ORGANIZATIONAI";
      font-size: 8pt;
      font-family: 'Inter', sans-serif;
      color: #64748b;
      font-weight: 500;
...
def generate_pdf():
    ...
    print("2. Converting English Markdown to HTML (TOC with dotted leaders, justified text)...")
    ...
    if res.returncode == 0 and os.path.exists(PDF_PATH):
        ...
        print(f"🎉 SUCCESS! English Proposal PDF generated at:")
